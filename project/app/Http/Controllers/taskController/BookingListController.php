...
{   
    ...
    public function bookingListView(){
        ...
        foreach($bookingList as &$booking){
            ...
            $booking->po = MxpIpo::where('booking_order_id', $booking->booking_order_id)->select(DB::Raw('GROUP_CONCAT(DISTINCT ipo_id) as ipo_id'))->groupBy('booking_order_id')->first();
            $booking->bookingDetails = MxpBooking::where('booking_order_id', $booking->booking_order_id)->select(DB::Raw('GROUP_CONCAT(DISTINCT poCatNo) as po_cat'))->groupBy('item_code')->first();
        }
